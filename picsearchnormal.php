...
<div class="main"> 
 <form action="picsearchnormal.php " method="POST">
...
$started =microtime(true);
  ...
  $query= "SELECT a.id, a.fname,a.lname,a.ffname,a.mfname,a.dob,  a.gender,
    a.address,a.weight,a.birthplace, a.birthtime,a.maxloc,a.t4,a.t3,a.t5,a.t2,a.sound,a.email,a.image_name,a.signup_date,

               CONCAT(GROUP_CONCAT(b.FILE_NAME)) FILE_NAME
FROM users a INNER JOIN upload_data b
ON a.id = b.id
WHERE a.id = '$valueToSearch'
 ...
 //echo $t;
    ...
    $sound=$row[16];
            ...
            $image_name=$row[18];
             $signup_date=$row[19];
              $FILE_NAME=$row[20];
            $splittedstring=explode(",",$FILE_NAME);
             
             ?>
             <table align="center" >  <caption><center><strong> Search by Conventional Method </strong></center></caption>
   ...
   <tr><td></td><td></td><td></td><td rowspan="6"><img src="saveimages/<?php echo $image_name;?>" width="120" height="120" style="border:1px solid black;"></td></tr>
      ...
      echo "<img src=\"nail_data/" . $value. "\" height= 145 width = 160 hspace=5/>";
        echo'<div style="font-size:11px;padding:7px;">';
        echo wordwrap($value,27,"<br>\n",TRUE);
        echo'</div>';
     echo  "</td>";
...
//echo $ac;
       ...
       <?php     
         ...
         //for claasification              
           ...
           $class2 = substr($FILE_NAME,(strpos($FILE_NAME, '_') ?: -1) + 1);
           $class = substr($class2, 0, strpos($class2, '_'));



           $query2="SELECT * FROM `upload_data` WHERE (`FILE_NAME`) LIKE '%".$class."%'";
           $result2= mysqli_query($con,$query2);
           $newendtime = microtime(true);
           //Calculate the difference in microseconds.
$diff = $newendtime - $newstarttime;

//Format the time so that it only shows 10 decimal places.
$qtime = number_format($diff, 10);
 $u= 1000000 * ($qtime);
// echo $u;             ?>
    ...
    $count = 0;
      ...
      $pieces = explode(" ", $total);
            ...
            foreach ($pieces as $key => $piece){
                      ...
                      echo "<td>";
                  echo "<img src=\"nail_data/" . $piece. "\" height= 145 width = 160 hspace=5/>";
        echo'<div style="font-size:11px;padding:7px;">';
        echo wordwrap($piece,27,"<br>\n",TRUE);
        echo'</div>';
...
//echo $rr;
        ?>
            ...
            <?php
             $z=$t+$ac+$u+$rr;
            echo "<center><br><br>SQL query took $z microseconds to execute";
